...
function encrypt(text, key, isPub) {
  ...
  var encryptKey;
  if (isPub) {
    encryptKey = ursa.createPublicKey(key);
    return encryptKey.encrypt(text, 'utf8', 'base64');
  } else {
    encryptKey = ursa.createPrivateKey(key);
    return encryptKey.privateEncrypt(text, 'utf8', 'base64');
  }
}
...
function decrypt(text, key, isPub) {
  ...
  var decryptKey;
  if (isPub) {
    decryptKey = ursa.createPublicKey(key);
    return decryptKey.publicDecrypt(text, 'base64', 'utf8');
  } else {
    decryptKey = ursa.createPrivateKey(key);
    return decryptKey.decrypt(text, 'base64', 'utf8');
  }
}
...
function test() {
  // 必须使用Pem格式
  var keyPair = {
    prvKeyObj: ursa.createPrivateKey(PRIVATE_KEY_PEM),
    pubKeyObj: ursa.createPublicKey(PUBLIC_KEY_PEM)
  };
  ...
  var encryptText = encrypt(plainText, PUBLIC_KEY_PEM, true);
  console.log(plainText, encryptText);
  var decryptText = decrypt(encryptText, PRIVATE_KEY_PEM, false);
  console.log(plainText, decryptText);

  var plainText = 'Java中文2';
  var encryptText = encrypt(plainText, PRIVATE_KEY_PEM, false);
  console.log(plainText, encryptText);
  var decryptText = decrypt(encryptText, PUBLIC_KEY_PEM, true);
  console.log(plainText, decryptText);
}
